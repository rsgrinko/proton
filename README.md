...
config/               config.php, menu.php, permissions.php, events.php, webhooks.php,
                      schedule.php, commands.php
...
**Вебхуки**: событие приложения уходит наружу подписчику — посылкой POST с подписью
и повторами. Подписки заводятся в панели, события перечисляются в `config/webhooks.php`:
