...
Микрофреймворк на PHP 8.1+ без composer и внешних зависимостей: нужны только
`pdo_sqlite` или `pdo_mysql`, `openssl`, `mbstring` и `json`.

Что есть: маршрутизация с прослойками, Active Record со связями и миграциями,
роли с правами и областью видимости, панель управления, API с ключами, очередь
задач с воркером и расписанием, почта (пять драйверов), вебхуки, уведомления,
резервные копии, настройки через панель, консоль с генераторами, свой тестраннер.
Тестов 151, гоняются на отдельной базе.

Одинаково работает на Linux (nginx + php-fpm) и на Windows
(`php bin/proton serve`).

Зависимостей нет сознательно: проект переносится копированием каталога, версии
ничего не ломают. Обратная сторона — всё, что нужно, приходится писать самому,
и этот код лежит в `framework/`.
...
накатывает схему, заводит администратора. После этого закрывается.
...
Событие можно отдать наружу подписчику (вебхук) или человеку внутри (уведомление),
оба идут через ту же очередь:
...
запись в контроллер не попадает.
