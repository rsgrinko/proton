...
Базу копирует сам Proton — ни `mysqldump`, ни `gzip` в системе не нужны:

```dotenv
BACKUP_SCHEDULE=03:30    # воркер сделает копию каждый день в это время
BACKUP_KEEP=7            # и оставит семь последних
```

Копии ложатся в `var/backups`, каждая сразу проверяется; негодная не сохраняется.
Вручную — `php bin/proton backup:create`, список — `backup:list --check`.
Подробности и восстановление: [BACKUP.md](BACKUP.md).

Этого мало для спокойной жизни: копии лежат на той же машине. Забирайте их наружу
по расписанию, например так:

```cron
30 4 * * * scp /var/www/proton/var/backups/$(ls -t /var/www/proton/var/backups | head -1) backup@другой-сервер:/backups/proton/
```

Кроме базы копировать нужно `var/storage` с загруженными файлами. `.env` стоит
хранить отдельно и надёжно: без `APP_KEY` зашифрованные значения из копии базы не
прочитать — ни секреты вебхуков, ни чужие ключи.
