...
и отчёты о доставке остаются на его стороне. Запрос идёт через общий
`Support\HttpClient`, и `curl` не обязателен — без расширения уходит потоками.

Проверку сертификата клиент не отключает. Там, где у PHP нет своего хранилища
корневых сертификатов (обычное дело на Windows), путь к набору задаётся настройкой —
иначе отправка падает на «SSL certificate problem: unable to get local issuer
certificate»:

```dotenv
HTTP_CA_BUNDLE=C:/путь/к/cacert.pem   # взять: https://curl.se/ca/cacert.pem
```

Тот же набор подхватывает и SMTP-драйвер при `STARTTLS`.
