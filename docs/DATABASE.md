...
в одну запись. Под своим ключом живут все варианты запроса: страницы `paginate()`
и `first()` рядом с `get()` не путаются между собой, а `Cache::forget()` по ключу
сбрасывает их разом. Пустой ответ (`first()` без строк) кэшируется так же, как
непустой. Кэшируются `get()`, `count()`, `sum()`, `max()` и всё, что
...
Пустой `$fillable` значит «ничего нельзя», а не «всё можно»: `create()`, `fill()`
и `update()` у такой модели молча отбросят все поля. Так забытый список не
открывает все колонки формы разом, но и заготовка `make:model` со своим пустым
`[]` ничего не сохранит, пока поля не перечислены.
...
имя маршрута. Разворачивается кликом, дальше — вкладки (на радиокнопках, без
JavaScript: переключаются и если скрипты не выполнились):

- **Запрос** — метод и путь, маршрут и его прослойки, кто сейчас смотрит (с
  поправкой на «вход под пользователем» — видно и того, кем притворились, и
  того, кто нажал кнопку), адрес клиента, `Content-Type`, `Request-Id`,
  `User-Agent`, имена полей запроса (без значений — они могут нести пароль);
- **Выполнение** — прослойки и контроллер таймлайном: отступ и положение внутри
  родительской полосы показывают вложенность, ширина — сколько занял слой
  вместе со всем, что внутри него, файл под названием — какой класс реально
  выполнился (имя прослойки типа `auth` само по себе не говорит, какой класс
  за ней стоит);
- **SQL** — таймлайн запросов (полоса на каждый: положение и ширина — момент
  начала и длительность в процентах от времени страницы, цвет — быстрый/на
  грани/медленный по тому же порогу, что и лог медленных запросов), повторы
  (один SQL несколько раз за страницу — почти всегда N+1, который стоит
  вынести в `with()`), самые долгие с числом строк в ответе, и полный список
  по порядку — свёрнут по умолчанию. У каждого запроса раскрывается стек
  вызова (файл:строка, без аргументов — та же причина, что и с параметрами);
- **Кэш** — попадания и промахи `Support\Cache` по каждому обращению за
  запрос, включая то, что прошло через `Query::remember()`.
...
перекрывает последнюю строку таблицы или кнопку. Раскрытая, область вкладок —
фиксированной высоты и скроллится сама: переключение вкладок не меняет высоту
панели.

Данные собирает `Support\Profiler`, имя текущего маршрута — `Router::current()`,
сам запрос для вкладки «Запрос» — `Request::current()` (тот же приём, что у
маршрута: вьюха рендерится внутри контроллера и своего доступа к запросу не
имеет). Своих SQL-параметров и значений полей формы панель не показывает
нарочно: они могут нести пароль или токен, а панель видна всем, кто открыл
страницу при включённом `APP_DEBUG` — поэтому на бою этот флаг обязательно
`false`.

Сама панель не рендерится там же, где стоит `View::partial('profiler')` в
layout: вместо неё в HTML остаётся заглушка `Profiler::PLACEHOLDER`, а
`Kernel::handle()` меняет её на готовую разметку самым последним шагом — когда
ответ уже прошёл через все прослойки обратно и у каждой известна отметка
выхода. Без этого добрая половина вкладки «Выполнение» была бы не видна:
то, что происходит после рендера вида (выход из прослоек, `Metrics::track`,
`HitWorker::run()`), к моменту вставки панели в шаблон ещё не случилось бы.

Данные показываются только для запроса, который их создал: у редиректа нет
тела и панели, поэтому событие или задача очереди, поставленные формой перед
уходом на другую страницу, в панели не всплывают — смотреть их стоит там, где
они происходят на самом деле (лог, `queue:status`), а не через эту панель.
