...
вместе со всей группой. `can:` берёт `viewer` из атрибутов запроса — его кладут
`auth` (сессия) и `api` (владелец ключа), — и только если его там нет, спрашивает
сессию. На API-маршрутах поэтому `can:` ставится после `api`, иначе проверялся бы
браузер, а не ключ.
...
Полное перекрытие годится для замены разметки целиком, но для одной строки —
дорого: скопировали `layouts/main.php` ради своих стилей, и следующее обновление
ядра (новая вкладка в панели отладки, поправленная разметка меню) в свою копию
само не попадёт — придётся сверять руками. Для точечных добавлений в каркас
ядра есть слоты — `View::fillSlot()` регистрирует разметку, `View::slot()` в
шаблоне её отдаёт (или пусто, если никто не заполнил):

```php
// bootstrap.php или config/services.php — код приложения, layouts/main.php не трогаем
View::fillSlot('layout.head', '<style>.sparkline { color: var(--accent); }</style>');
```

Слоты каркаса на сегодня: `layout.head` (перед `</head>`, свои стили или мета-теги)
и `layout.topbar` (в шапке, рядом со ссылкой «Уведомления»).
