...
и уводит на главную. Отдельной страницы первого запуска нет. Недоступная база — это
503, а не «не установлено»: иначе сбой MySQL открывал бы мастер любому прохожему.
...
`can:` берёт `viewer` из атрибутов запроса, поэтому на API он проверяет ключ, а не
сессию — там его ставят после `api`. **Нельзя выдать больше своего**: роль, права
роли, правка человека, вход под ним и ключ на его имя — только если его права
целиком покрыты своими (`Viewer::covers()`, `Role::assignableBy()`).
...
Долгая задача объявляет свой `timeout()`: по его истечении её сочтут зависшей и
вернут в очередь, а исчерпавшую попытки — похоронят с вызовом `failed()`.
