...
/**
 * Приём чужих вебхуков: POST /api/v1/hooks/{источник}.
 *
 * Ключ API здесь не нужен — чужая система его не знает; вместо него подпись,
 * которую проверяет `Incoming`. Отвечаем быстро и коротко: отправитель ждёт
...
final class HooksController extends ApiController
{
    public function receive(Request $request, string $source): Response
    {
        ...
        return match ($status) {
            ...
            IncomingHook::REJECTED => Response::error('Подпись не сошлась', 401),
