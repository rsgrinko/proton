...
final class RateLimiter
{
    ...
    public function attempt(string $key, int $max, int $seconds): bool
    {
        ...
        // Сначала отметка, потом сверка: между «проверил» и «отметил» соседний
        // запрос успевал проскочить, и под нагрузкой предел превышался. Счётчик
        // увеличивается одним запросом в базе, так что после hit() число честное
        $this->hit($key, time() + $seconds);

        return $this->count($key) <= $max;
    }
