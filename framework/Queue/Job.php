...
abstract class Job
{
    ...
    /**
     * Сколько секунд задача может работать, прежде чем её сочтут зависшей
     * (воркер убит) и вернут в очередь. Долгие задачи — копия большой базы,
     * выгрузка — поднимают срок у себя: иначе второй воркер запустит их
     * повторно, пока первый ещё работает.
     */
    public function timeout(): int
    {
        return max(1, (int) Config::get('queue.stuck_minutes', 15)) * 60;
    }
