...
final class Scheduler
{
    ...
    public static function run(): array
    {
        ...
        foreach (self::$tasks as $name => $task) {
            ...
            // Срок перепроверяем уже под блокировкой: соседний воркер мог
            // выполнить задачу между нашей проверкой выше и захватом — тогда
            // отметка уже свежая, и второй прогон был бы лишним
            if (!self::due($name, $task)) {
                $lock->release();
